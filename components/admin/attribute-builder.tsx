...
export function AttributeBuilder({ productId, initialAttributes = [] }: { productId?: string; initialAttributes?: Attribute[] }) {
  ...
  function save() { startTransition(async () => { const form = new FormData(); if (productId) { form.set("productId", productId); form.set("attributes", JSON.stringify(attributes)); setStatus((await saveProductAttributes(form)).ok ? "Product attributes saved." : "Unable to save product attributes."); } else { form.set("name", groupName); form.set("values", groupValues); setStatus((await createAttributeGroup(form)).ok ? "Global attribute group saved." : "Unable to save attribute group."); } }); }
